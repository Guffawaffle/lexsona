...
export function formatProvenance(
  ...
  if (mode === "compact") {
    ...
    const sourceCode =
      provenance.source === "persona" ? "p" : provenance.source === "learned" ? "r" : "b";
...
export function formatConstraint(
  ...
  if (provMode === "compact" && c.provenance) {
    ...
    // Return a new object with the compact provenance, typed as any to bypass strict checking
    // since we're intentionally mixing formats
    const result = {
      ...c,
 ...
 */
export function formatPrinciple(p: Principle, format: OutputFormat): Principle | CompactPrinciple {
  if (format === "compact") {
    return {
      id: p.id,
      desc: p.description,
